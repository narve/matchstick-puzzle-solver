...
- `match.js` — rules engine + UI wiring. `getRuleSets()` returns the three
  rulesets (`strict`, `default`, `flexible`). `setup()` runs on page load,
  builds radios from `getRuleSets()`, renders samples + rules table, wires
  the input.
- `matchstick-svg.js` — SVG rendering of every character. `injectDefs()`
  registers a `<symbol>` per legal char (union across rulesets) inside a
  hidden `<svg id="char-defs">`. `charSvg(c, w, h)` and `equationSvg(text,
  h)` produce `<svg><use href="#c-…">` strings.
- `samples.js` — sample puzzles tagged by ruleset and difficulty.
- `index.html` — page shell + styles. Almost all rendering is JS-driven.
...
mutate }` (see the JSDoc typedef in `match.js`). Three rulesets ship today,
...
- `defaultEvaluate` (canonicalises before `eval`)
- input normalisation in `putSample` (so `<input>` shows canonical text
  even when the clicked solution had alt chars)
- the rules-table renderer skips alt-character rows because every rule
  involving an alt form is already present in the canonical row
